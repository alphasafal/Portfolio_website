...
export function AuroraVeins() {
  ...
  useEffect(() => {
    ...
    const pathEls = svg.querySelectorAll<SVGPathElement>(".aurora-vein-path");
    const triggers: ScrollTrigger[] = [];
    const maxOpacity = window.innerWidth < 768 ? 0.2 : 0.35;

    pathEls.forEach((path, i) => {
      const len = path.getTotalLength();
      gsap.set(path, { strokeDasharray: len, strokeDashoffset: len, opacity: 0.08 });
  ...
  return (
    <svg
      ref={svgRef}
      className="aurora-veins pointer-events-none absolute inset-0 z-[1] h-full w-full"
      viewBox="0 0 1440 9000"
      preserveAspectRatio="none"
      aria-hidden
    >
      <defs>
        <linearGradient id="aurora-vein-gradient" x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="0%" stopColor="#5683da" stopOpacity="0.85" />
          <stop offset="50%" stopColor="#8b7cff" stopOpacity="0.55" />
      ...
      {paths.map((d, i) => (
        <path
          key={i}
          d={d}
          className="aurora-vein-path"
          fill="none"
          stroke="url(#aurora-vein-gradient)"
          strokeWidth={1.5}
          strokeLinecap="round"
        />
      ))}
